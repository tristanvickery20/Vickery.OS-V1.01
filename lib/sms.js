...
// Consult-request admin notification templates.
// CONSULT_REQUEST_ADMIN: sent to OWNER_PHONE when a customer requests a consult
//   from the quote wizard (manual-quote-only service).
//   Vars: {{name}}, {{phone}}, {{service}}, {{best_time}}
const CONSULT_TEMPLATES = {
  CONSULT_REQUEST_ADMIN:
    "[Consult Request] {{name}} \u2014 {{phone}}\nService: {{service}}\nBest time to call: {{best_time}}\nRespond via CRM or call back directly.",
};

module.exports = { sendSms, buildMessage, CREW_TEMPLATES, INVOICE_TEMPLATES, RESCHEDULE_TEMPLATES, CONSULT_TEMPLATES };
