...
function secret() { return process.env.SESSION_SECRET || "vickery-crew-fallback"; }
...
async function readAllStaff() {
  ...
  if (!sid) return [];
  const sheets = await getSheetsClient();
  const resp = await sheets.spreadsheets.values.get({ spreadsheetId: sid, range: `${STAFF_TAB}!A:J` });
...
async function sendSms(to, body) {
  ...
  if (!sid || !token || !from) {
    console.log("[staff/sms] Twilio not configured — SMS not sent:", body.slice(0, 80));
    return false;
  }
    ...
    const req = https.request({
      ...
      r.on("end", () => {
        if (r.statusCode >= 200 && r.statusCode < 300) {
          console.log("[staff/sms] Sent to", to);
          resolve(true);
        } else {
          console.error("[staff/sms] Failed:", r.statusCode, data.slice(0, 200));
          resolve(false);
        }
      });
    });
    req.on("error", err => { console.error("[staff/sms] Error:", err.message); resolve(false); });
    req.write(postBody);
    req.end();
  });
}
...
module.exports = {
  ensureStaffSheet, readAllStaff, findStaffByPhone, findStaffById,
  appendStaffRow, updateStaffRow, pendingCount,
  hashPassword, normalizePhone,
