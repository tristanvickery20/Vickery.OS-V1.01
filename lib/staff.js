...
// ── Helpers ───────────────────────────────────────────────────────────────────
// Staff tab lives in the HR sheet (migrated from CRM in Task #39/40).
function spreadsheetId() { return hrSpreadsheetId(); }
...
// Like readAllStaff but distinguishes a sheet-read failure (quota, network) from a
// genuinely empty staff list. Returns { rows, degraded } so callers can surface an
// appropriate error to the user instead of falling through to wrong-username logic.
async function readAllStaffSafe() {
  const sid = spreadsheetId();
  if (!sid) return { rows: [], degraded: false };
  try {
    const sheets = await getSheetsClient();
    const resp = await sheets.spreadsheets.values.get({ spreadsheetId: sid, range: `${STAFF_TAB}!A:Z` });
    const raw = resp.data.values || [];
    if (raw.length < 2) return { rows: [], degraded: false };
    const headers = raw[0];
    ...
    console.error("[staff/readAllStaffSafe] Sheet read failed:", err.message);
...
async function appendStaffRow(staff) {
  const sid = spreadsheetId();
  if (!sid) throw new Error("No HR_SHEET_ID");
  const sheets = await getSheetsClient();
...
module.exports = {
  ensureStaffSheet, readAllStaff, readAllStaffSafe, findStaffByPhone, findStaffByUsername, findStaffById,
