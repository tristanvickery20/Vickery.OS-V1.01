...
const CLAIM_ITEM_HEADERS = [
  "item_id","claim_id","category","amount","receipt_url","description","created_at",
];
...
async function activeAssignment(staffId) {
  const list = await readAssignmentsForEmployee(staffId);
  const today = new Date().toISOString().slice(0, 10);
  return list.find(a => a.effective_date <= today) || null;
}
...
async function updateRun(id, updates)     { return updateRowById(RUN_TAB, "run_id", id, updates); }
 ...
 */
async function computeEmployeePay(staffId, periodStart, periodEnd, workingDays, totalDays, additions) {
  const assignment = await activeAssignment(staffId);
  if (!assignment) return null;

  const base = parseFloat(assignment.base_amount) || 0;
  ...
  for (const si of structureItems) {
    ...
    if (comp.value_type === "Fixed") {
      ...
      // Simple formula evaluation: supports "base * 0.1" style expressions
      try {
        const formulaStr = (comp.formula || "0").replace(/base/gi, String(base));
  ...
  // Add one-off additions/deductions from the run
  ...
  const grossEarnings = base + earnings.reduce((s, e) => s + e.amount, 0);
