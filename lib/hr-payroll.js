...
// All HR_ExpenseClaims*, HR_Salary*, and HR_Payroll* tabs now live on HR_SHEET_ID.
// Copy all those tabs from the CRM sheet to the HR sheet manually on first boot.
...
async function ensurePayrollSheets() {
  if (!process.env.HR_SHEET_ID) {
    throw new Error("[FATAL] HR_SHEET_ID is required. Payroll sheets cannot initialize without it.");
  }
