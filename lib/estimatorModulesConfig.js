...
// SEED_SERVICE_MIN = 35 matches the 35 rows seeded: 34 active services + 1 disabled
// seed-version marker (_SEED_VER_20260512 in estimatorSeedData.js). On first boot the
// sheet has 34 rows (< 35) → stale → clears both tabs and re-seeds with 35 rows.
// After re-seed: 35 rows ≥ 35 → stable, no further clearing.
//
// To force a future re-seed: add a new disabled version-marker service to SERVICES in
// estimatorSeedData.js (e.g. _SEED_VER_20260601), then bump SEED_SERVICE_MIN to 36.
// The sheet will have 35 rows < 36 → stale → clears and re-seeds with 36 rows → stable.
const SEED_MODULE_MIN  = 35;
const SEED_SERVICE_MIN = 35;
