...
// When ESTIMATOR_V2_SHEET_ID is not set, falls back to the embedded seed data
// so the full 54-service / 56-module quiz works without a separate sheet.
...
// ── Seed-data fallback (no sheet required) ────────────────────────────────────
// Builds the exact same shape as loadConfigFromSheet() but from the local
// MODULES / SERVICES constants so the quiz works even without ESTIMATOR_V2_SHEET_ID.
...
async function getEstimatorConfig() {
  ...
  const useSheet = Boolean(process.env.ESTIMATOR_V2_SHEET_ID);
  _cache   = useSheet ? await loadConfigFromSheet() : loadConfigFromSeed();
  _cacheAt = Date.now();
  return _cache;
}
