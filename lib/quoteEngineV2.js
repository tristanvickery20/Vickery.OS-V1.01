...
// Stabilization: flagged outlier materials excluded from rawMaterialCost computation.

const { isFlagged } = require("./flaggedMaterials");
const { ZONE_FEES, ZONE_RULES } = require("./serviceArea");
...
function calculateQuoteV2({ assembly, qty, selectedDriverOptions, v2Data, serviceAreas, zip, stackCap }) {
  ...
  // Flagged outlier materials are excluded from instant pricing (zero contribution).
  ...
  for (const item of items) {
    ...
    if (!mat) continue;
    if (isFlagged(item.item_ref_id)) {
      const contribution = n(item.qty_per_unit) * qty * n(mat.base_cost);
      if (contribution > 0) {
        excludedMaterials.push({ id: item.item_ref_id, contribution });
        console.warn(`[quoteV2] Excluded flagged material ${item.item_ref_id} from assembly ${assembly.assembly_id} (contribution $${contribution.toFixed(2)})`);
      }
      continue;
    }
  ...
  if (address_provided) {
    ...
    if (area) {
      resolvedZone = String(area.zone || "").toUpperCase().trim();
      travelFee = resolvedZone in ZONE_FEES ? ZONE_FEES[resolvedZone] : (Number(area.travel_fee) || 25);
      const zoneRule = ZONE_RULES[resolvedZone];
      if (zoneRule) {
        custom_quote_only = zoneRule.custom_quote_only || false;
        if (zoneRule.approval_note) travel_note = zoneRule.approval_note;
      }
    } else {
      resolvedZone = "EXT";
      travelFee  = 25;
