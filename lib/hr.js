...
// Departments (HR_Departments) and Designations (HR_Designations) now live on HR_SHEET_ID.
// The Staff tab remains on CRM_SHEET_ID for scheduling and crew lookups.
// Copy HR_Departments and HR_Designations from the CRM sheet to the HR sheet manually on first boot.
...
function crmId() { return process.env.CRM_SHEET_ID; }

// ── Generic tab helpers (HR sheet) ─────────────────────────────────────────────
...
// ── Employee HR profile migration (CRM sheet — Staff tab stays on CRM) ──────────
...
// ── Employee profile read/update (CRM sheet) ───────────────────────────────────
...
// ── Append a full employee row (CRM sheet) ─────────────────────────────────────
...
async function ensureHrSheets() {
  if (!process.env.HR_SHEET_ID) {
    throw new Error("[FATAL] HR_SHEET_ID is required. HR module cannot initialize without it.");
  }
  await Promise.all([
