...
function evaluateService(modulesById, service, answersByModule, photoCount) {
  ...
  if (ceiling === "vaulted") {
    disqualified = true;
    reasons.push("Vaulted/high ceilings require site assessment.");
  }
 ...
 * Returns { final_price, source } where source is "v2" or "placeholder".
 */
async function getBasePrice(serviceId, serviceName, qty) {
  try {
    ...
    return { final_price: result.final_price || 0, source: "v2" };
  } catch {
