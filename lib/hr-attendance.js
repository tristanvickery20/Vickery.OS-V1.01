...
// ── Sync from Time log ──────────────────────────────────────────────────────────
// Reads the Time sheet and marks any employee with a time entry on a given date as Present.
// Resolves tech_id (which is stored as a crew member's full name) to the canonical
// staff_id UUID by matching against the Staff sheet's first_name + last_name columns.

async function syncAttendanceFromTimeLog(targetDate) {
  const spreadsheetId = sid();
  if (!spreadsheetId) return { synced: 0 };
  const sheets = await getSheetsClient();

  // Load Time, Staff, and ShiftTypes sheets in parallel
  const [timeResp, staffResp] = await Promise.all([
    sheets.spreadsheets.values.get({ spreadsheetId, range: "Time!A1:L5000" }),
    sheets.spreadsheets.values.get({ spreadsheetId, range: "Staff!A:AZ" }),
  ]);
  ...
  // Load shift types for off-shift detection
  ...
  nameSet.forEach(name => {
    ...
    // If name not found in Staff sheet, skip — can't safely map to a UUID
  });

  let synced = 0;
  for (const { staffId, defaultShiftId } of resolved) {
    const existing = await readAttendanceByStaffAndDate(staffId, dateStr);
    // Skip only if an admin-approved correction has already set this record.
    if (existing && existing.source === "correction") continue;

    // Detect off-shift: if employee has a default shift with days_of_week, check if target date's day is covered.
