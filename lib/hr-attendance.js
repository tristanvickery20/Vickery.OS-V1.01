...
async function syncAttendanceFromTimeLog(targetDate) {
  ...
  // Load Time and Staff from CRM sheet
  const [timeResp, staffResp] = await Promise.all([
    sheets.spreadsheets.values.get({ spreadsheetId: crmSpreadsheetId, range: "Time!A1:L5000" }),
    sheets.spreadsheets.values.get({ spreadsheetId: crmSpreadsheetId, range: "Staff!A:AZ" }),
  ]);
