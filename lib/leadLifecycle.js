...
async function mirrorBookingToLead({ sheets, spreadsheetId, quoteId, leadId, snapshot = {}, booking = {}, desiredStatus = "Scheduled" }) {
  ...
  put(row, idx, "last_quote_id", qid, { onlyBlank: false });
  put(row, idx, "name", booking.customer_name || snapshot.customer_name, { onlyBlank: false });
  put(row, idx, "phone", booking.phone || snapshot.phone, { onlyBlank: false });
  put(row, idx, "email", booking.email || snapshot.email, { onlyBlank: false });
  put(row, idx, "address", booking.address || snapshot.address, { onlyBlank: false });
  put(row, idx, "job_type", booking.job_type_id || snapshot.job_type_id, { onlyBlank: false });
  put(row, idx, "quoted_price", booking.final_price || snapshot.final_price, { onlyBlank: false });
  put(row, idx, "estimated_value", booking.final_price || snapshot.final_price, { onlyBlank: false });
  put(row, idx, "pricing_version", snapshot.pricing_version || booking.pricing_version, { onlyBlank: false });
  put(row, idx, "quote_snapshot_json", buildQuoteSnapshotJson(snapshot, { booking_id: booking.booking_id || "", lead_id: finalLeadId }), { onlyBlank: false });
...
module.exports = { mirrorBookingToLead, enrichBookingsFromLeads, buildQuoteSnapshotJson };
