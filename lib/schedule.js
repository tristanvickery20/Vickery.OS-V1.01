...
function buildPersonalBusyMinutes(personalBusyBlocks, targetDate, tz) {
  ...
  for (const b of personalBusyBlocks || []) {
    ...
    // Check if the block falls on targetDate in the CRM's local timezone
    const blockLocalDate = localDateInTZ(b.start, timezone);
    if (blockLocalDate !== targetDate) continue;
    blocks.push({
      start: parseTimeToMinutesInTZ(b.start, timezone),
      end:   parseTimeToMinutesInTZ(b.end,   timezone),
    });
  }
  return blocks;
}
