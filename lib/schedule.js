...
function localDateInTZ(dateTimeStr, tz) {
  ...
  try {
    ...
    // Access each part by type explicitly — avoids relying on separator order
    // or locale-specific literal characters that differ across Node.js versions.
...
// Map personal GCal busy blocks to minute-ranges for targetDate (IANA tz required).
// Includes cross-midnight events that started on the previous day but end on targetDate.
