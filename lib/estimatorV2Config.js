...
async function loadV2Config() {
  ...
  const jobTypes = assemblyRows
    ...
    .map(a => {
      ...
      const durMin    = Math.max(60, Math.ceil((baseHours * 60) / 30) * 30);
      return {
        job_type_id:              a.assembly_id,
        name_public:              a.niche_name || a.assembly_id,
        segment:                  a.serviceType || "Residential",
        category:                 a.category    || "",
  ...
  const allDrivers = driverRows
    ...
    .map(d => ({
      ...
      prompt:      d.prompt || d.driver_id,
      input_type:  d.input_type || "radio",
      required:    toBool(d.required),
      sort_order:  toNum(d.sort_order),
    }))
  ...
  for (const m of multiplierRows) {
    ...
    optionsByQuestion[m.driver_id].push({
      option_id:    m.multiplier_id || m.option_id || (m.driver_id + "_" + optionsByQuestion[m.driver_id].length),
      label:        m.label || "",
      effect_type:  "MULTIPLY_HOURS",
      effect_value: toNum(m.labor_multiplier || "1"),
    });
  }

  // ── ServiceAreas from CRM sheet ───────────────────────────────────────────
  const serviceAreas = {};
  for (const r of areaRows) {
    if (!r.zip) continue;
    serviceAreas[r.zip.trim()] = { zone: r.zone, travel_fee: toNum(r.travel_fee) };
  }
