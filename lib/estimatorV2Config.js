...
// Loads Instant Estimator v2 sheet, caches, and returns the config shape used by the rest of the stack.
// ESTIMATOR_V2_SHEET_ID is REQUIRED — the V1 fallback engine has been removed.
// If ESTIMATOR_V2_SHEET_ID is not set, calls to getActiveConfig() throw a fatal error.
...
const SHEET_ID_V2  = () => {
  ...
  if (!id) throw new Error("[FATAL] ESTIMATOR_V2_SHEET_ID is not configured. The V1 estimator has been removed. Set this environment variable.");
  return id;
};
...
async function loadV2Config() {
  ...
  const [
    ...
    readTab(sheets, v2Id,  "ServiceAreas").catch(() => []),
  ]);
  ...
  const SERVICE_TO_CATEGORY = {
    ...
    MOTION_SECURITY_LIGHT:      "LIGHTING",
    // ── Fans ──────────────────────────────────────────────────────────────────
  ...
  return {
    // Public shape — identical to what getQuoteConfig() returned
    jobTypes,
    questionsByType,
    optionsByQuestion,
    addonsByType: {},
...
/**
 * Returns the active V2 quote config.
 * Throws a fatal error if ESTIMATOR_V2_SHEET_ID is not set — V1 has been removed.
 */
async function getActiveConfig() {
  if (!process.env.ESTIMATOR_V2_SHEET_ID) {
    throw new Error(
      ...
      "Configure this environment variable to point to your V2 estimator Google Sheet."
    );
  }
  ...
  if (!cfg.jobTypes || cfg.jobTypes.length === 0) {
    throw new Error("[EstimatorV2] Assemblies tab is empty or unreadable. Check ESTIMATOR_V2_SHEET_ID and sheet permissions.");
  }
  _cache   = cfg;
  _cacheAt = Date.now();
  return _cache;
