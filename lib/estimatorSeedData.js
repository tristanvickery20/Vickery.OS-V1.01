...
const SERVICES = [
  ...
  },
  {
    ...
    // quantity (high-volume tube jobs), replace vs new run (2nd fork), height + access (biggest labor drivers — note:
    // ATTIC_ACCESS mainly relevant for recessed_kit; conditional logic deferred), wet/outdoor spec, age, room, photo, buffer
    // PRODUCT_TYPE removed — generic fixture options (flush_mount/chandelier) don't fit LED retrofit context
...

  {
    ...
    segment:      "residential",
    tier:         "instant",
    // A004 — 10 questions: existing power (new circuit?), box rated? (safety gate), ceiling height + access (labor),
    // outdoor/damp spec (before features), customer-supplied, controls, 3-way, home age (pre-1950 affects safe circuit load), photo
    // NOTE: FAN_RATED_BOX is moot if EXISTING_WIRING = "no" — reorder/conditional logic deferred to future pass
