...
const MODULES = [
...

  {
    ...
    input_type: "single_select",
    options_json: JSON.stringify([
      { value: "lt9",   label: "Standard — 8 to 10 ft",   multiplier: 1.0,  disqualify: false },
      { value: "9_12",  label: "Tall — 10 to 15 ft",      multiplier: 1.25, disqualify: false },
      { value: "12_14", label: "Vaulted — 15 to 20 ft",   multiplier: 1.55, disqualify: false },
      { value: "15_20", label: "Very high — over 20 ft",  multiplier: 1.0,  disqualify: true  },
    ]),
    applies_to: "lighting,fans,detectors",
    notes: "Values match MODULE_OVERRIDES in quote.js (lt9/9_12/12_14/15_20). 15_20 triggers site visit. 12_14 (vaulted) priceable with tall ladder.",
  },
...

  {
    ...
    question:   "Is there already a switch or outlet at this exact location?",
    input_type: "single_select",
    options_json: JSON.stringify([
      { value: "device_swap",     label: "Yes — I'm replacing the existing one at the same spot",        multiplier: 0.45 },
      { value: "extend_existing", label: "No — new spot, but there's an outlet or switch nearby",         multiplier: 1.0  },
      { value: "new_circuit",     label: "No — nothing nearby, needs new wiring run from the panel",      multiplier: 1.5  },
    ]),
...

  {
    ...
    question:   "Is there already a ceiling fan, light, or electrical box up there where the fan will go?",
    input_type: "single_select",
    options_json: JSON.stringify([
      { value: "yes_fan_box",   label: "Yes — ceiling fan or light is already there (just replacing it)", multiplier: 1.0  },
      { value: "yes_light_box", label: "Yes — a light fixture is there, but no fan has ever been there",  multiplier: 1.1  },
      { value: "no_box",        label: "No — blank ceiling, nothing there at all",                        multiplier: 1.55 },
    ]),
    applies_to: "fans",
...

  {
    ...
    question:   "Are you replacing something at the same spot, or adding something to a brand new location?",
    input_type: "single_select",
    options_json: JSON.stringify([
      { value: "replace",  label: "Replacing an existing one — same spot, same location",        multiplier: 0.6  },
      { value: "new_near", label: "New location — there's an outlet, switch, or light nearby",   multiplier: 1.0  },
      { value: "new_far",  label: "New location — nothing nearby, needs wire run from the panel", multiplier: 1.5  },
      { value: "not_sure", label: "Not sure — I'd like your team to advise",                     multiplier: 1.1, uncertain: true },
    ]),
    applies_to: "all",
...

  {
    ...
    question:   "Is there an outlet, light switch, or light fixture near where you need work done?",
    input_type: "single_select",
    options_json: JSON.stringify([
      { value: "yes_full",   label: "Yes — right at that spot (outlet, switch, or light is already there)", multiplier: 1.0  },
      { value: "yes_nearby", label: "Yes — one is nearby, same room or within a few feet",                  multiplier: 1.15 },
      { value: "no",         label: "No — nothing nearby, would need new wiring from the panel",             multiplier: 1.45 },
      { value: "not_sure",   label: "Not sure — I'm not sure what's in the walls",                          multiplier: 1.1, uncertain: true },
    ]),
    applies_to: "all",
  ...
  {
    module_id:  "_SEED_VER_20260513",
    question:   "Seed version marker — do not display",
    input_type: "single_select",
    options_json: "[]",
    applies_to: "_none",
    notes: "Version marker to force re-seed when SEED_MODULE_MIN is bumped. Fixes CEILING_HEIGHT option IDs (lt9/9_12/12_14/15_20).",
  },
];
...
const SERVICES = [
  ...
  {
    service_id:   "_SEED_VER_20260512E",
