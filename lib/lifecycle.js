...
function makeIssue({ key, stage, severity, title, count, reason, href, status }) {
  return {
    key,
    stage,
...
function activeBookingKey(booking) {
  return firstValue(booking, ["lead_id", "quote_id", "booking_id", "id"]);
}
...
function buildCloseoutChecklist(ctx = {}) {
  ...
  const checks = [
    ...
    makeIssue({
      ...
      count: scheduledMissingAssignment,
  ...
  if (hasAnyHeader(dataPresence.bookingsData, ["customer_confirmed", "confirmation_status", "confirmed_at"])) {
    ...
    checks.push(makeIssue({
      key: "scheduled_missing_confirmation",
      stage: "Scheduled → Prepped",
      severity: "warning",
      title: "Scheduled work is missing customer confirmation",
      count: missingConfirmation,
      reason: "Customer confirmation helps prevent wasted drive time and schedule chaos.",
      href: "/crm/schedule",
    }));
  } else {
  ...
  if (hasAnyHeader(dataPresence.leadsData, ["prep_status", "materials_status", "materials_ready", "prep_ready", "prepped_at"])) {
    ...
    checks.push(makeIssue({
      ...
      stage: "Scheduled → Prepped",
      severity: "warning",
      title: "Scheduled work is missing prep/materials readiness",
      count: missingPrep,
      reason: "Jobs should be checked for materials and scope before the crew arrives.",
      href: "/crm/schedule",
    }));
  } else {
  ...
  checks.push(
    ...
    completedMissingExpenses === null
      ...
      : makeIssue({
          ...
          stage: "Complete → Invoiced",
          severity: "info",
          title: "Completed work has no expense/material record",
          count: completedMissingExpenses,
          reason: "Expense/material records help preserve job-costing history. Some small jobs may legitimately have none.",
          href: "/crm/expenses",
        })
  );
  ...
  if (hasAnyHeader(dataPresence.leadsData, ["completion_notes", "final_notes", "closeout_notes", "final_photo_url", "completion_photo_url"])) {
    ...
    checks.push(makeIssue({
      ...
      stage: "Complete → Invoiced",
      severity: "warning",
      title: "Completed work is missing final notes/photos",
      count: missingFinalProof,
      reason: "Closeout proof protects the business if there is a callback or dispute.",
      href: "/clients",
    }));
  } else {
  ...
  if (hasAnyHeader(dataPresence.leadsData, ["review_requested_at", "review_request_sent_at", "review_status"])) {
    ...
    checks.push(makeIssue({
      ...
      severity: "warning",
      title: "Review request is missing",
      count: missingReview,
      reason: "Closed or paid jobs should trigger a review request when appropriate.",
      href: "/reviews",
    }));
  } else {
  ...
  if (hasAnyHeader(dataPresence.leadsData, ["retention_status", "retention_next_touch_at", "follow_up_date", "retention_handoff_at"])) {
    ...
    checks.push(makeIssue({
      ...
      severity: "info",
      title: "Retention handoff is missing",
      count: missingRetention,
      reason: "Closed jobs should land in a future follow-up or retention path.",
      href: "/clients",
    }));
  } else {
