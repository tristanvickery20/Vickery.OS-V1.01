...
// Share both managed calendars (Jobs + Internal) with a user's Google account.
// This is how the CRM owner makes the calendars appear in their own Google Calendar / iPhone.
// role: "writer" lets the owner add/edit events directly; "reader" is view-only.
async function shareCalendarsWithUser(email, role) {
  if (!email || !email.includes("@")) throw new Error("Enter a valid Gmail or Google Workspace address.");
  const ids = await loadCalendarIds();
  if (!ids.jobs && !ids.internal) {
    throw new Error("Calendars have not been bootstrapped yet. Run 'Bootstrap Calendars' first.");
  }
  const cal     = await getCalendarClient();
  const useRole = role || "writer";
  const results = [];

  for (const [label, calendarId] of [["Vickery Jobs", ids.jobs], ["Vickery Internal", ids.internal]]) {
    if (!calendarId) continue;
    try {
      await cal.acl.insert({
        calendarId,
        requestBody: { role: useRole, scope: { type: "user", value: email } },
      });
      results.push({ label, ok: true });
      console.log(`[gcal] Shared "${label}" with ${email} (${useRole})`);
    } catch (err) {
      // 409 = already shared — treat as success
      const status = err.code || (err.response && err.response.status);
      if (status === 409) {
        results.push({ label, ok: true, note: "already shared" });
      } else {
        results.push({ label, ok: false, error: err.message });
        console.error(`[gcal] Share "${label}" with ${email} failed:`, err.message);
      }
    }
  }
  return results;
...
// ─────────────────────────────────────────────
// Backfill: push all existing CRM items that have no gcal_event_id yet
// ─────────────────────────────────────────────
async function backfillToGCal() {
  const ids = await loadCalendarIds();
  if (!ids.enabled) throw new Error("Google Calendar sync is not enabled. Enable it in Settings first.");
  ...
  // ── 1. LEADS ─────────────────────────────────────────────────────
  try {
    const resp = await sheets.spreadsheets.values.get({ spreadsheetId, range: "Leads!A1:AZ5000" });
    const rows = resp.data.values || [];
    const headers = rows[0] || [];
    const gcalCol  = headers.indexOf("gcal_event_id");

    for (let i = 1; i < rows.length; i++) {
      const row = rows[i] || [];
      const id            = String(row[0] || "").trim();
      const leadName      = String(row[2]  || "").trim();
      const leadAddr      = String(row[4]  || "").trim();
      const jobType       = String(row[5]  || "").trim();
      const scheduledDate = String(row[13] || "").trim();
      const leadNotes     = String(row[15] || "").trim();
      const durMins       = Math.max(30, Number(row[18]) || 120);
      const existing      = gcalCol >= 0 ? String(row[gcalCol] || "").trim() : "";

      if (!id || !scheduledDate || existing) { skipped++; continue; }

      const titleSuffix = jobType ? `${jobType} – ${leadName}` : leadName;
      const gcalTitle = `Job – ${titleSuffix}${leadAddr ? " – " + leadAddr.split(",")[0] : ""}`;

      try {
        await sleep(300); // avoid rate limit
        const result = await createJobGCalEvent({
          title: gcalTitle, status: "Scheduled",
          scheduledDate, durationMinutes: durMins,
          address: leadAddr, notes: leadNotes,
        });
        if (result && result.gcalEventId) {
          await writeGCalEventIdToSheet({
            tab: "Leads", idCol: 0, idValue: id,
            gcalCol, gcalEventId: result.gcalEventId, calendarId: result.calendarId,
          });
          pushed++;
        }
      } catch (e) { console.error("[gcal/backfill] Lead", id, e.message); errors++; }
    }
  } catch (e) { console.error("[gcal/backfill] Leads read error:", e.message); }
  ...
  try {
    ...
    for (let i = 1; i < rows.length; i++) {
      const row = rows[i] || [];
      const task_id  = String(row[0] || "").trim();
      const title    = String(row[1] || "").trim();
      const type     = String(row[2] || "Task").trim();
      const due_date = String(row[3] || "").trim();
      const notes    = String(row[7] || "").trim();
      const existing = String(row[11] || "").trim();

      if (!task_id || !due_date || existing) { skipped++; continue; }

      const dueDateStr = due_date.slice(0, 10);
      try {
        await sleep(300);
      ...
      } catch (e) { console.error("[gcal/backfill] Task", task_id, e.message); errors++; }
    }
  } catch (e) { console.error("[gcal/backfill] Tasks read error:", e.message); }

  // ── 3. EVENTS ─────────────────────────────────────────────────────
  try {
    const resp = await sheets.spreadsheets.values.get({ spreadsheetId, range: "Events!A1:K5000" });
    const rows = resp.data.values || [];
    for (let i = 1; i < rows.length; i++) {
      const row       = rows[i] || [];
      const event_id  = String(row[0] || "").trim();
      const title     = String(row[1] || "").trim();
      const type      = String(row[2] || "Meeting").trim();
      const startDT   = String(row[3] || "").trim();
      const endDT     = String(row[4] || "").trim();
      const notes     = String(row[6] || "").trim();
      const existing  = String(row[10] || "").trim();

      if (!event_id || !startDT || existing) { skipped++; continue; }

      try {
        await sleep(300);
        const result = await createGCalEvent({
          title, type, startDT, endDT: endDT || startDT, notes, isAllDay: false,
        });
        if (result && result.gcalEventId) {
          await writeGCalEventIdToSheet({
      ...
      } catch (e) { console.error("[gcal/backfill] Event", event_id, e.message); errors++; }
    }
  } catch (e) { console.error("[gcal/backfill] Events read error:", e.message); }

  console.log(`[gcal/backfill] Done — pushed=${pushed} skipped=${skipped} errors=${errors}`);
