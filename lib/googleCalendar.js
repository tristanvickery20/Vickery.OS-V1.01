...
function calendarForType(type) {
  ...
  // "Personal Block" events go to Internal (service account has write access there).
  // The personal calendar is strictly READ-ONLY (free/busy access only) — creating events
  // there would require calendar.acl write permission which the personal calendar owner
  // would not typically grant. Personal Block in Internal still blocks scheduling slots
  // and shows on the owner's phone once they share Vickery Internal with their account.
  if (t === "personal block") return "internal";
