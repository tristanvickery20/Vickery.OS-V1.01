...
async function bootstrapCalendars() {
  try {
    ...
    // Auto-generate a webhook channel token on first bootstrap if not already stored.
    // This token must be set on watch channel requests so the webhook handler can verify
    // the notification is genuine.  Visible in /api/gcal/status as webhookToken.
    if (!cfg[CFG_WEBHOOK_TOKEN]) {
      const token = crypto.randomUUID();
      await writeConfig(CFG_WEBHOOK_TOKEN, token);
      console.log("[gcal] Generated webhook channel token (set on watch channel requests)");
    }

    _calCache = null; // force reload
    console.log(`[gcal] Calendars ready — Jobs: ${jobsId} | Internal: ${internalId}`);

    // Validate personal calendar connection if an ID has been configured.
    // The personal calendar is user-owned — the service account can't create it.
    // Bootstrap verifies write access so Personal Block events can be created.
    let personalStatus = "not_configured";
    const personalId = cfg[CFG_PERSONAL_ID] || "";
    if (personalId) {
      try {
        // Try a minimal event insert/delete to confirm write access (writer permission required)
        const testEvent = {
        ...
        const ins = await cal.events.insert({ calendarId: personalId, requestBody: testEvent });
        if (ins.data.id) {
          await cal.events.delete({ calendarId: personalId, eventId: ins.data.id }).catch(() => {});
        }
        personalStatus = "connected";
        console.log(`[gcal] Personal calendar verified (write access) — ${personalId}`);
      } catch (pErr) {
...
function calendarForType(type) {
  ...
  // "Personal Block" events are written directly to the owner's personal calendar.
  // This requires the personal calendar to be shared with the service account with
  // "Make changes to events" (writer) permission — see Settings → Connect Personal Calendar.
      ...
      if (leadGcalCol >= 0) {
        for (let ri = 1; ri < leadsVals.length; ri++) {
          ...
          if (deleted) {
            ...
            // Covers both Scheduled and In Progress leads that have a GCal event ID
...
/**
 * Verify the service account can read freebusy from the configured personal calendar.
 * Returns { ok, message } — call this from the settings UI "Test Connection" button.
 */
async function verifyPersonalCalendar(calendarId) {
  if (!calendarId) throw new Error("No personal calendar ID configured.");
  const cal = await getCalendarClient(); // uses same JWT auth as the rest of the module
  const now  = new Date();
...
async function runOverdueTaskAlerts() {
  try {
    ...
    // Use CRM timezone (from Config) so "today" is the correct local calendar date,
    // not the UTC date (which may differ on Replit's UTC server around midnight).
...
// ─────────────────────────────────────────────
// Schedules runOverdueTaskAlerts() to fire at 8:05 AM in the CRM timezone
// (from ids.timezone / Config sheet, default America/Chicago).
// Falls back to UTC 8:05 if Config is unavailable.
async function scheduleDailyOverdueAlerts() {
  ...
  function msUntilNext8_05() {
    const now = new Date();
    // Get current local date + time in the CRM timezone
    const parts = new Intl.DateTimeFormat("en-US", {
      timeZone: tz, hour12: false,
    ...
    // Pick today or tomorrow depending on whether 8:05 AM has already passed
