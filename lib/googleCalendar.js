...
function remindersForType(type) {
  ...
  if (t === "task")           return [{ method: "popup", minutes: 480 }]; // 8h = 8 AM same day if due at 4 PM; best approximation
...
async function runReverseSync() {
  ...
  try {
    ...
    const sheets = await getSheetsClient();
    const spreadsheetId = SPREADSHEET_ID();
    const leadsResp = await sheets.spreadsheets.values.get({ spreadsheetId, range: "Leads!A1:Z5000" });
    const eventsResp = await sheets.spreadsheets.values.get({ spreadsheetId, range: "Events!A1:Z5000" });
    ...
    for (const gEvt of updatedEvents) {
      ...
      if (leadGcalCol >= 0) {
        for (let ri = 1; ri < leadsVals.length; ri++) {
          ...
          if (deleted) {
            // Lead deleted from GCal → flag as Unscheduled
            if (leadStatusCol >= 0 && String(row[leadStatusCol]) === "Scheduled") {
              ...
              console.log(`[gcal reverse] Lead row ${ri + 1} deleted from GCal → Unscheduled`);
            }
