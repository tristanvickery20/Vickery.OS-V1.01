...
// Tabs live on the Ops sheet (TRUCK_STOCK_ID): FleetVehicles, FleetMaintenance,
// ToolAssets, ToolIssues, InventoryItems, TruckStock, MaterialRequests
const { getSheetsClient, colToLetter } = require("./sheets");
const { opsSpreadsheetId } = require("./opsSheetClient");
...
async function ensureTab(tabName, headers) {
  ...
  if (!id) throw new Error("TRUCK_STOCK_ID env var is required");
