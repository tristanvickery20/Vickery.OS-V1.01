  ...
  async getTransactions({ startDate, endDate, maxResults } = {}) {
    try {
      ...
      const purchases = (qr?.Purchase || []).map((p) => {
        // PrivateNote format for CRM-pushed entries:
        //   "CRM Expense EXP-xxx — Category: Foo — Payee Name — actual notes"
        const rawNote = p.PrivateNote || "";
        let category  = "";
        let parsedPayee = "";
        let cleanNote  = rawNote;

        if (rawNote.startsWith("CRM Expense ")) {
          // Strip the "CRM Expense EXP-xxx — " prefix
          const afterId = rawNote.replace(/^CRM Expense \S+ — /, "");

          // Check for "Category: Foo — " pattern
          const catRe = /^Category:\s*(.+?) — ([\s\S]*)$/;
          const catMatch = afterId.match(catRe);
          if (catMatch) {
            category = catMatch[1].trim();
            const afterCat = catMatch[2];
            // Next segment is the payee, everything after is the actual note
            const dashIdx = afterCat.indexOf(" — ");
            if (dashIdx >= 0) {
              parsedPayee = afterCat.slice(0, dashIdx).trim();
              cleanNote   = afterCat.slice(dashIdx + 3).trim();
            } else {
              parsedPayee = afterCat.trim();
              cleanNote   = "";
            }
          } else {
            // No category — first segment is payee, rest is notes
            const dashIdx = afterId.indexOf(" — ");
            if (dashIdx >= 0) {
              parsedPayee = afterId.slice(0, dashIdx).trim();
              cleanNote   = afterId.slice(dashIdx + 3).trim();
            } else {
              parsedPayee = afterId.trim();
              cleanNote   = "";
            }
          }
        }
        ...
        return {
          ...
          payee:        p.EntityRef?.name || parsedPayee,
          category,
          type:         "purchase",
        };
      });
