...
const SCHEMAS = {
  ...
  // Task #75 — Proof Asset Library
  MarketingAssets: [
    "asset_id", "created_at",
    "job_id", "lead_id", "client_id",
    "service_type", "city",
    "asset_type",          // before_photo|after_photo|team_photo|truck_photo|panel_photo|generator_photo|fixture_photo|testimonial|review_screenshot|job_story
    "file_url", "file_type",
    "date_taken",
    "marketing_use_allowed",  // yes|no|pending
    "permission_status",      // not_asked|asked|granted|denied
    "best_for_gbp", "best_for_website", "best_for_social", "best_for_ads",
    "notes",
  ],

  // Task #75 — Contractor Task Board
  ContractorTasks: [
    "task_id", "created_at",
    "task_name", "contractor_name",
    "contractor_type",    // google_ads|seo_gbp|website_cro|photographer|csr|print_offline|va
    "channel", "campaign_id",
    "due_date",
    "status",             // pending|in_progress|done|blocked|cancelled
    "output_expected", "link_or_file",
  ...
  // ─── Campaign Management (Task #72) ──────────────────────────────────────
  Campaigns: [
    "campaign_id", "name", "channel", "source", "service_promoted",
    "city_zone_targeted", "start_date", "end_date", "budget", "spend",
    "landing_page", "tracking_phone", "utm_campaign", "offer_message",
    "contractor_vendor", "status", "owner_decision", "notes", "created_at",
  ],
  LandingPages: [
    "page_id", "url", "title", "service_type", "city", "utm_campaign",
...
const MARKETING_TABS = new Set(["Reviews", "Templates", "ReferralLedger", "MarketingAssets", "ContractorTasks", "Campaigns", "LandingPages"]);
...
async function patchSchedulerRuleDefaults() {
  ...
  try {
    ...
    if (ltIdx >= 0 && (ltVal === "0" || ltVal === "" || ltVal === "4" || !ltVal)) {
      ...
      console.log(`[SchedulerRules] lead_time_hours patched ${ltVal || "blank"}→3 at startup.`);
    }
  } catch (err) {
