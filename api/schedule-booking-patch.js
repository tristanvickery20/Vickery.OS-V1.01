...
async function resolveCrewNamesByIds(sheets, spreadsheetId, ids) {
  ...
  const staffResp = await sheets.spreadsheets.values.get({ spreadsheetId, range: "Staff!A:Z" }).catch(() => ({ data: { values: [] } }));
...
async function appendTechAssignedSnapshot({ sheets, spreadsheetId, updatedBooking }) {
  ...
  const quoteRows = rows.slice(1).map((r) => Object.fromEntries(headers.map((h, i) => [h, r[i] || ""]))).filter((r) => String(r.quote_id||"").trim() === String(updatedBooking.quote_id||"").trim());
  const bestSnap = quoteRows.find((r) => r.event_type === "locked") || quoteRows[quoteRows.length - 1] || {};
  let selectedQty = 1;
  let materialTotals = [];
  try {
    const so = JSON.parse(bestSnap.selected_options_json || "{}");
    selectedQty = Math.max(1, Number(so.qty || 1) || 1);
    const equip = Array.isArray(so.equipment_line_items) ? so.equipment_line_items : [];
    materialTotals = equip.map((li) => {
      const unitQty = Number(li.quantity || li.qty || 1) || 1;
      return {
        name: li.title || li.name || "Material",
        unit_qty: unitQty,
        job_qty: selectedQty,
        total_qty: unitQty * selectedQty,
  ...
  const snapshotState = { ...updatedBooking, assigned_crew_names: assignedCrewNames, selected_job_qty: selectedQty, material_totals: materialTotals };
